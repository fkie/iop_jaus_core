...
//	printf("[Events] query message %#x for update is %ssupported\n", messageid, (supported) ? "" : "not " );
	...
	if (event->getRequestedPeriodicRate() < MINIMUM_RATE || event->getRequestedPeriodicRate() > MAXIMUM_RATE) {
		supported = false;
...
Events_ReceiveFSM::EventData::EventData(jUnsignedShortInteger query_msg_id, Receive::Body::ReceiveRec transportData)
{
	...
	sender.setSubsystemID(transportData.getSrcSubsystemID());
	sender.setNodeID(transportData.getSrcNodeID());
	sender.setComponentID(transportData.getSrcComponentID());
	seq_nr = 0;
}
