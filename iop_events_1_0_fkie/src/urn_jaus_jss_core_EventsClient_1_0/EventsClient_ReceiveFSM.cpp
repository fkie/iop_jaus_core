...
EventsClient_ReceiveFSM::EventsClient_ReceiveFSM(urn_jaus_jss_core_Transport_1_0::Transport_ReceiveFSM* pTransport_ReceiveFSM)
{
...
void EventsClient_ReceiveFSM::handleRejectEventRequestAction(RejectEventRequest msg, Receive::Body::ReceiveRec transportData)
{
	...
	pRemoveEventRequestHandler(request_id);
	mutex.unlock();
	if (p_class_events_reply_callback) {
...
{
	std::map <jUnsignedByte, boost::function<void (Receive::Body::ReceiveRec &transport_data, Event &msg)> >::iterator it;
...
void EventsClient_ReceiveFSM::pRemoveEventConfirmedHandler(JausAddress &address, jUnsignedByte event_id)
{
	...
	std::map <std::pair<jUnsignedByte, unsigned int>, boost::function<void (Receive::Body::ReceiveRec &transport_data, Event &msg)> >::iterator it;
...
{
	uint16_t subsystem_id = transport_data.getSrcSubsystemID();
	uint8_t node_id = transport_data.getSrcNodeID();
	uint8_t component_id = transport_data.getSrcComponentID();
	JausAddress sender(subsystem_id, node_id, component_id);
	std::map <std::pair<jUnsignedByte, unsigned int>, boost::function<void (Receive::Body::ReceiveRec &transport_data, Event &msg)> >::iterator it;
	it = p_callbacks_confirmed.find(std::make_pair(msg.getBody()->getEventRec()->getEventID(), sender.get()));
	if (it != p_callbacks_confirmed.end()) {
		it->second(transport_data, msg);
	}
}
