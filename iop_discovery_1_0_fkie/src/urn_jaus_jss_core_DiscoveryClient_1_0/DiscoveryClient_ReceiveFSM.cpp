...
#include "urn_jaus_jss_core_DiscoveryClient_1_0/DiscoveryClient_ReceiveFSM.h"
...
namespace urn_jaus_jss_core_DiscoveryClient_1_0
{

DiscoveryClient_ReceiveFSM::DiscoveryClient_ReceiveFSM(urn_jaus_jss_core_Transport_1_0::Transport_ReceiveFSM* pTransport_ReceiveFSM, urn_jaus_jss_core_EventsClient_1_0::EventsClient_ReceiveFSM* pEventsClient_ReceiveFSM)
{
...
void DiscoveryClient_ReceiveFSM::reportedConfigurationAction(ReportConfiguration msg, Receive::Body::ReceiveRec transportData)
{
	ROS_DEBUG_NAMED("DiscoveryClient", "reportedConfigurationAction from %d.%d.%d, not implemented", transportData.getSrcSubsystemID(), transportData.getSrcNodeID(), transportData.getSrcComponentID());
...
void DiscoveryClient_ReceiveFSM::reportedIdentificationAction(ReportIdentification msg, Receive::Body::ReceiveRec transportData)
{
	mutex_.lock();
	...
	ros_msg.address.subsystem_id = transportData.getSrcSubsystemID();
	ros_msg.address.node_id = transportData.getSrcNodeID();
	ros_msg.address.component_id = transportData.getSrcComponentID();
	...
	if (p_enable_ros_interface or pHasToDiscover()) {
		if (p_enable_ros_interface) {
			p_pub_identification.publish(ros_msg);
		}
		// request configuration
		QueryConfiguration req_cfg_msg;
		JausAddress addr(transportData.getSrcSubsystemID(), transportData.getSrcNodeID(), transportData.getSrcComponentID());
...
void DiscoveryClient_ReceiveFSM::reportedServicesAction(ReportServices msg, Receive::Body::ReceiveRec transportData)
{
	/// Insert User Code HERE
	...
	if (p_enable_ros_interface) {
		...
		node_addr.subsystem_id = transportData.getSrcSubsystemID();
		node_addr.node_id = transportData.getSrcNodeID();
		node_addr.component_id = transportData.getSrcComponentID();
		ReportServices::Body::NodeList *node_list = msg.getBody()->getNodeList();
		for (unsigned int n = 0; n < node_list->getNumberOfElements(); n++) {
			ReportServices::Body::NodeList::NodeSeq *nodes = node_list->getElement(n);
			ReportServices::Body::NodeList::NodeSeq::NodeRec *node = nodes->getNodeRec();
	...
	for (int i = 0; i < p_discover_services.size(); i++) {
		if (p_discover_services[i].subsystem == transportData.getSrcSubsystemID() && !p_discover_services[i].discovered) {
...
void DiscoveryClient_ReceiveFSM::reportedSubsystemListAction(ReportSubsystemList msg, Receive::Body::ReceiveRec transportData)
{
	/// Insert User Code HERE
...
void DiscoveryClient_ReceiveFSM::sendRegisterServicesAction(ReportIdentification msg, Receive::Body::ReceiveRec transportData)
{
	...
	ROS_DEBUG_NAMED("DiscoveryClient", "sendRegisterServices query_type: %d, sender: subsystem: %d, node: %d,	component: %d",
			query_type, transportData.getSrcSubsystemID(), transportData.getSrcNodeID(), transportData.getSrcComponentID());
	if (p_own_uri_services.size() == 0) {
		// do not register, if we have no services to register setted by appendServiceUri()
		reportedIdentificationAction(msg, transportData);
		return;
	}
	if (query_type == discovery_config::TYPE_SUBSYSTEM) {
		if (this->jausRouter->getJausAddress()->getSubsystemID() == transportData.getSrcSubsystemID()) {
			if (!onRegistration() && !isRegistered()) {
				p_on_registration = true;
				p_addr_discovery_service = JausAddress(transportData.getSrcSubsystemID(),
...
void DiscoveryClient_ReceiveFSM::discover(std::string service_uri, int major_version, int minor_version, int subsystem)
{
	p_count_discover_tries = 0;
