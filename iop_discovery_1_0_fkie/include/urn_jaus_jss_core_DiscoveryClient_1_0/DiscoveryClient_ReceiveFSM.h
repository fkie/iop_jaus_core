...
#include "urn_jaus_jss_core_DiscoveryClient_1_0/Messages/MessageSet.h"
#include "urn_jaus_jss_core_DiscoveryClient_1_0/InternalEvents/InternalEventsSet.h"
...
public:
	...
	int TIMEOUT_STANDBY;
	DiscoveryClient_ReceiveFSM(urn_jaus_jss_core_Transport_1_0::Transport_ReceiveFSM* pTransport_ReceiveFSM, urn_jaus_jss_core_EventsClient_1_0::EventsClient_ReceiveFSM* pEventsClient_ReceiveFSM);
	...
	virtual void reportedConfigurationAction(ReportConfiguration msg, Receive::Body::ReceiveRec transportData);
	virtual void reportedIdentificationAction(ReportIdentification msg, Receive::Body::ReceiveRec transportData);
	virtual void reportedServicesAction(ReportServices msg, Receive::Body::ReceiveRec transportData);
	virtual void reportedSubsystemListAction(ReportSubsystemList msg, Receive::Body::ReceiveRec transportData);
	...
	void discover(std::string service_uri, int major_version=1, int minor_version=0, int subsystem=65535);
	template<class T>
	void discover(std::string service_uri, void(T::*handler)(const std::string &, JausAddress &), T*obj, int major_version=1, int minor_version=0, int subsystem=65535)
	{
		...
		this->discover(service_uri, major_version, minor_version, subsystem);
	}
